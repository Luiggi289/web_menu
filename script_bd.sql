...
CREATE TABLE IF NOT EXISTS wp_user_copydish (
id_user bigint  NOT NULL,
flag_active_dish boolean,
flag_active_dish_ingredient boolean,
flag_active_dish_part_day boolean
);
...
INSERT INTO wp_day(name) VALUES (7,'Domingo');
...
-- insert copia 
insert into `wp_dish` ( name,id_type_dish,description ,id_user,id_reference ) SELECT name,id_type_dish,description , 3 id_user ,id id_reference FROM `wp_dish` WHERE id_user=1;

insert into `wp_dish_part_day` 
(id_dish,id_part_day)
SELECT a.id , c.id_part_day  FROM `wp_dish` a
inner join `wp_dish` b on a.id_reference=b.id 
inner join `wp_dish_part_day` c on b.id=c.id_dish
WHERE a.id_user=3;


insert into `wp_dish_ingredient` 
(id_dish,sequence,id_ingredient,id_unit_measure,quantity)
SELECT a.id , 
c.sequence,c.id_ingredient,c.id_unit_measure,c.quantity
FROM `wp_dish` a
inner join `wp_dish` b on a.id_reference=b.id 
inner join `wp_dish_ingredient` c on b.id=c.id_dish
WHERE a.id_user=3;
